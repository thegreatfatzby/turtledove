...
loading FLEDGE resources. The `trustedBiddingSignalsURL` must also not have a [query](https://url.spec.whatwg.org/#concept-url-query). (See bullets 5.11 and 5.12 [of spec section 2.1](https://wicg.github.io/turtledove/#joining-interest-groups)).

The `renderUrl` property of an `ad` must be also be a valid and credentialless HTTPs URL, but does _not_
have the same origin, response header, fragment, or query requirements (bullet 5.15 of [section 2.1](https://wicg.github.io/turtledove/#joining-interest-groups)).
